...
final class ChannelSiteLinkSetTest extends TestCase {
	public function testItWillCorrectlyDeserialize() {
		$timestamp = new DateTimeField('2022-02-22T22:22:22+0000');
		...
		$data = [
			...
			'id' => $id->toString(),
			'timestamp' => $timestamp->object->format(DateTimeInterface::RFC3339_EXTENDED),
			'payload' => [
		...
		$this->assertEquals($expected, ConnectorEvent::deserializeValue($data));
	}
}
