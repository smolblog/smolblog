...
final class StandardContentProjectionTest extends TestCase {
	...
	public function testItAddsANewExtension() {
		...
		$event = new readonly class(
			...
			public function __construct(private ContentExtension $ext, mixed ...$props) { parent::__construct(...$props); }
		...
		$this->assertOnlyTableEntryEquals(
			...
			publish_timestamp: null,
			visibility: ContentVisibility::Draft->value,
			extensions: json_encode([get_class($ext) => $ext->serializeValue()]),
		);
	}
	...
	public function testItAddsASecondExtension() {
		...
		$this->assertOnlyTableEntryEquals(
			...
			publish_timestamp: null,
			visibility: ContentVisibility::Draft->value,
			extensions: json_encode(['std_class' => ['one' => 'two'], get_class($ext) => $ext->serializeValue()]),
		);
	}
	...
	public function testItReplacesAnExistingExtension() {
		...
		$this->db->table('standard_content')->update(['extensions' => json_encode([get_class($ext) => $ext->serializeValue()])]);
		...
		$this->assertOnlyTableEntryEquals(
			...
			publish_timestamp: null,
			visibility: ContentVisibility::Draft->value,
			extensions: json_encode([get_class($ext) => $newExt->serializeValue()]),
		);
	}
